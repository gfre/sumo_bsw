 ...
 *
 * Predictor
 * x_apri = A*x_apost(-)  + B*u(k) ,
 * P_apri = A*P_apost(-)*A^T + Q ,
 *
 * Corrector
 * K(k)       = P_apri*C^T * (C*P_apri*C^T + R)^{-1} ,
 * x_apost(+) = x_apri + K(k)*( y(k) - C*x_apri ) ,
 * P_apost(+) = (E - K(k)*C) * P_apri.
 *
 *
 * Dimensions
 *  x    n-by-1 state vector,
 *  A    n-by-n system/state transition matrix,
 *  P(k) n-by-n error covariance matrix,
 *  u(k) l-by-1 input vector to the system system,
 *  B    n-by-l input matrix (l being the number of inputs),
 *  y(k) m-by-1 measurement vector,
 *  C    m-by-n measurement matrix ('m' being the number of measured states),
